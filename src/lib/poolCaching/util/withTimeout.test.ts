...
describe('withTimeout', () => {
  ...
  it('rejects when promise exceeds timeout', async () => {
    ...
    await expect(withTimeout(slowPromise, 50, 'test')).rejects.toThrow('Timeout after 50ms [test]');
  });
  ...
  it('includes label in timeout error message', async () => {
    ...
    await expect(withTimeout(slowPromise, 10, 'myLabel')).rejects.toThrow('[myLabel]');
  });
