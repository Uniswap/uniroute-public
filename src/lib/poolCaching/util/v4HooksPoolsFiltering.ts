...
function isHooksPoolRoutable(pool: V4SubgraphPool, chainId: ChainId, logger: Logger, metric: IMetric): boolean {
  try {
...
export function v4HooksPoolsFiltering(chainId: ChainId, pools: Array<V4SubgraphPool>, logger: Logger, metric: IMetric): Array<V4SubgraphPool> {
  ...
  pools.forEach((pool: V4SubgraphPool) => {
    if (isHooksPoolRoutable(pool, chainId, logger, metric)) {
      ...
      if (
        ...
        pool.tvlETH = 826;
        pool.tvlUSD = 1482475;
        logger?.info(`Setting tvl for OPTIMISM ETH/WETH pool ${JSON.stringify(pool)}`);
      ...
      if (
        ...
        logger?.info(`Setting tvl for UNICHAIN ETH/WETH pool ${JSON.stringify(pool)}`);
        additionalAllowedPool += 1;
      }
      ...
      if (
        ...
        logger?.info(`Setting tvl for BASE ETH/WETH pool ${JSON.stringify(pool)}`);
        additionalAllowedPool += 1;
      }
      ...
      if (
        ...
        logger?.debug(`Setting tvl for ARBITRUM ETH/WETH pool ${JSON.stringify(pool)}`);
        additionalAllowedPool += 1;
      }
