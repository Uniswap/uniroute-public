...
describe('S3_POOL_CACHE_KEY', () => {
  ...
  it('works with different chains and protocols', () => {
    expect(S3_POOL_CACHE_KEY('base', ChainId.BASE, Protocol.V2)).toBe(
      'base-8453-V2'
    );
    expect(S3_POOL_CACHE_KEY('base', ChainId.ARBITRUM_ONE, Protocol.V4)).toBe(
      'base-42161-V4'
    );
  });
});
